...
export class LogActiveDayService {
	...
	async createOrUpdate({ sessionCount }: LogActiveDayDto, userId: number) {
		...
		/*return this.prisma.logActiveDay.upsert({
			create:{
				userId,
				sessionCount
			},
	...
	async getStatistics(userId: number) {
		return this.prisma
			.$queryRaw`SELECT TO_CHAR(DATE_TRUNC('month', created_at), 'Month') AS month, SUM(session_count) AS session_count FROM log_active_day WHERE user_id = ${userId} GROUP BY month ORDER BY month DESC`
	}
}
